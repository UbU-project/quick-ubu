...
use crate::{CompletionFact, Store};
...
pub fn report_by_category(
    ...
    // Storage supplies completion facts, with any corrections already applied.
    for CompletionFact { item_id, at, actual } in completions {
        if *at < from || *at > to {
            continue;
        }
